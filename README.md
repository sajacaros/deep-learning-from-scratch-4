...
각 장 폴더의 `README.md`에 그 장의 정리를 담았습니다. 책의 절 구성을 그대로 따르면서,
본문 그림·수식과 함께 **그림에서 무엇을 봐야 하는지**, 그리고 예제 코드의 **실제 실행 결과**를 실었습니다.

|장 |정리 |핵심 |
|:--|:--|:--|
|1 |[밴디트 문제](ch01/README.md) |활용과 탐색, 증분 평균, ε-탐욕, 고정값 α |
|2 |[마르코프 결정 과정](ch02/README.md) |상태·행동·전이·보상·정책, 수익과 가치 함수 |
|3 |[벨만 방정식](ch03/README.md) |벨만 방정식과 벨만 최적 방정식, Q 함수 |
|4 |[동적 프로그래밍](ch04/README.md) |정책 반복법, 가치 반복법 |
|5 |[몬테카를로법](ch05/README.md) |모델 없이 경험으로, 오프-정책과 중요도 샘플링 |
|6 |[TD법](ch06/README.md) |SARSA, **Q 러닝** |
|7 |[신경망과 Q 러닝](ch07/README.md) |DeZero, `Q`를 신경망으로 근사 |
|8 |[DQN](ch08/README.md) |경험 재생, 목표 신경망, 카트 폴 |
|9 |[정책 경사법](ch09/README.md) |REINFORCE, 베이스라인, 행위자-비평자 |
|10 |[한 걸음 더](ch10/README.md) |알고리즘 분류, 레인보우, 사례와 남은 숙제 |

## 파일 구성

|폴더 이름 |설명                         |
|:--        |:--                          |
|ch01       |1장에서 사용하는 소스 코드 + 보충 예제 2개 ※원서에 없는 추가분 |
|ch02       |2장(마르코프 결정 과정) 보충 예제 ※원서에 없는 추가분 |
|ch03       |3장(벨만 방정식) 보충 예제 ※원서에 없는 추가분 |
|ch04~ch09  |각 장에서 사용하는 소스 코드   |
|ch10       |10장 정리(원서에도 소스 코드가 없는 장) |
...
원서 1장은 ε와 α를 각각 한 값으로 고정해두고 결과만 보여줍니다.
그 값을 바꾸면 무엇이 달라지는지 직접 확인해볼 수 있도록 비교 실험 두 개를 추가했습니다.
둘 다 원서 코드(`s02_bandit.py`, `s05_non_stationary.py`)의 클래스를 그대로 가져다 씁니다.

|파일 |대응 절 |내용 |
|:--|:--|:--|
|`ch01/s04_epsilon_comparison.py` |1.4 |ε를 0부터 1까지 바꿔가며 탐색과 활용의 균형을 비교. 순수 탐욕(ε=0)과 순수 무작위(ε=1)가 왜 둘 다 손해인지, 그리고 작은 ε가 큰 ε를 추월하는 시점이 언제인지 확인 |
|`ch01/s06_alpha_comparison.py` |1.5 |고정값 α를 바꿔가며 비교. α가 '느리고 안정적'과 '빠르고 불안정' 사이의 손잡이임을 보이고, 승률이 변하는 속도(σ)별로 최적 α가 어떻게 달라지는지 확인 |

읽는 순서는 `s03_bandit_avg.py` → `s04_epsilon_comparison.py`,
`s05_non_stationary.py` → `s06_alpha_comparison.py`처럼
원서 예제를 먼저 보고 이어서 보는 것을 권합니다.
