...
- 보충 예제: [`s02_discount.py`](s02_discount.py) — `γ`를 바꿔가며 수익을 비교
  - 주의: 아래 출력의 `mu1`/`mu2`는 스크립트가 붙인 이름으로, 2.4.2의 그림 2-16 번호와 순서가 반대임
    (출력의 `mu1` = 그림 2-16의 `μ2`, 출력의 `mu2` = 그림 2-16의 `μ1`). 아래에서는 행동 쌍으로 읽는 편이 안전함
  ```
  gamma에 따른 수익 G_0 (시작 상태 L1)
  ----------------------------------------------------------
    gamma  mu1 (L1:Right, L2:Left) mu2 (L1:Right, L2:Right)
  ----------------------------------------------------------
     0.00                   1.0000                   1.0000
     0.30                   1.0989                   0.5714
     0.50                   1.3333                   0.0000
...
- ![](images/s02_discount.png)
  ...
  - **오른쪽** — 수익의 부분합이 수렴하는 모습(정책 `(Right, Left)`)
