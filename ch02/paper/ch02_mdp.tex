...
한 가지 표기상의 주의가 필요하다. \texttt{s02\_discount.py}는 비교 대상 두 정책을 각각
\texttt{mu1 (L1:Right, L2:Left)}, \texttt{mu2 (L1:Right, L2:Right)}라는 이름으로 출력하는데, 이는
원서 그림 2-16의 번호와 순서가 반대다. 본고는 혼동을 피하기 위해 그림 2-16의 번호를 따르고 행동 쌍을
함께 병기하였다. 즉 본고의 $\mu_2$가 스크립트의 \texttt{mu1}이다.
