...
이 방법은 두 군데에서 막힌다. 하나는 \emph{세로}로 막힌다. 식~\eqref{eq:value-def}의 $G_t$는 무한급수이므로
정의대로는 끝까지 더할 수 없고, 500걸음에서 자르는 것은 어디까지나 근사다. 다른 하나는 \emph{가로}로
막힌다. 결정적 정책의 수는 $|\Aset|^{|\Sset|}$이므로 상태가 100개이고 행동이 4가지면
$4^{100}\approx1.6\times10^{60}$가지를 세어야 한다. 두 칸 세계에서 네 가지를 세어 본 것은 문제가 작았기
때문이지 방법이 좋았기 때문이 아니다.

그림~\ref{fig:backup-infinite}이 세로 방향의 벽을 그대로 보여준다. 앞 장에서 그린 백업 다이어그램인데,
...
2절에서 백업 다이어그램의 확률 셈법을 정리한 뒤 수익의 재귀 구조로부터 벨만 방정식을 도출한다.
3절은 이 식을 연립일차방정식으로 보고 두 칸 세계와 $3\times4$ 그리드에서 실제로 푼다.
4절에서 행동 가치 함수를 도입하여 $V$와 $Q$의 상호 변환을 정리하고, 5절에서 $\sum_a\pi(a\mid s)$를
$\max_a$로 바꾼 벨만 최적 방정식을 다룬다.

원서 3장에는 소스 코드가 없다. 본고가 추가한 것은 예제 네 편과 다음 네 가지 분석이다.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{손풀이의 경우 분석과 최적 연산자의 축소성}(\S\ref{sec:cases}, \S\ref{sec:contraction}).
        네 가지 결정적 정책 가정 중 자기일관인 것이 하나뿐임을 전수 확인하고, $\max$의 비확장성으로부터
        최적 벨만 연산자가 $\gamma$-축소 사상임을 증명한다. 예제가 정확히 333회에서 멈추는 이유도
        부동소수점 분해능으로 설명한다.
\end{enumerate}
...
다음 두 절에서 그 ``다른 방식''이 무엇인지, 그리고 정확해가 있는데도 왜 그쪽을 쓰는지 살펴본다.

\subsection{[보충] 반복 대입의 수렴률은 정확히 \texorpdfstring{$\gamma$}{gamma}다}
\label{sec:rate}

식~\eqref{eq:matrix-form}은 우변에 $v$가 있으므로 \emph{고정점 방정식}이기도 하다. 벨만 연산자
\begin{equation}
  (\Tpi v)(s) = \sum_a \pi(a\mid s)\sum_{s'} p(s'\mid s,a)\bigl\{r(s,a,s')+\gamma v(s')\bigr\},
  \qquad \Tpi v = r_\pi + \gamma P_\pi v
  \label{eq:operator}
\end{equation}
를 두면 벨만 방정식은 $v_\pi = \Tpi v_\pi$로 쓰인다. 그러면 아무 값에서 출발해 우변에 계속 대입하는
\begin{equation}
  v_{k+1} = \Tpi v_k = r_\pi + \gamma P_\pi v_k
  \label{eq:iteration}
\end{equation}
가 참값으로 수렴하리라 기대할 수 있다. 근거는 $\Tpi$가 축소 사상이라는 것이다.
...
\subsection{\texorpdfstring{$Q$}{Q} 함수의 정의}
...
지정되어 갈래가 하나로 고정되고 그 아래부터 $\pi$를 따른다. \bookfig{3-11}}
\label{fig:v-vs-q}
\end{figure}
...
\subsection{\texorpdfstring{$Q$}{Q}에 대한 벨만 방정식}
...
\subsection{\texorpdfstring{$\sum_a\pi(a\mid s)$}{sum\_a pi(a|s)}에서 \texorpdfstring{$\max_a$}{max\_a}로}
...
\end{equation}
$Q$에 대해서도 같은 치환이 성립한다. 식~\eqref{eq:q-bellman}의 $\sum_{a'}\pi(a'\mid s')$를
$\max_{a'}$로 바꾸면 된다.
\begin{equation}
...
\subsection{두 칸 세계의 최적 가치}

같은 두 칸짜리 그리드 월드로 돌아간다. 이번에는 정책이 주어지지 않는다.
...
앞 절의 손풀이는 ``$\Lone$에서 \texttt{Right}, $\Ltwo$에서 \texttt{Left}''라는 가정에서 출발했다. 이
가정이 맞았음을 사후 검증했지만, 다른 세 가정을 택했다면 어떤 일이 벌어졌을지는 확인하지 않았다.
결정적 정책이 네 가지뿐이므로 전부 풀어 볼 수 있다. 가정한 정책 $\mu$에 대해 $\max$를 제거한 연립
...
\end{equation}
로 두면, 벨만 최적 방정식은 $v_* = \Tstar v_*$라는 고정점 방정식이다. 핵심은 $\max$가 비확장적이라는
다음 보조 사실이다.
...
\texttt{s04\_bellman\_optimal.py}는 갱신량 \lstinline|delta|가 정확히 $0$이 될 때까지 돌아 333회에서
멈춘다. 이 숫자는 부동소수점 분해능에서 나온다. 수렴값 $v_*(\Lone)=5.2632$ 부근에서 배정밀도의 최소
간격(1\,ulp)은 $8.882\times10^{-16}$인데, 이론적 갱신량 $\gamma^k$가 이 값 아래로 내려가는 지점이
\begin{equation}
...
넷째, 손풀이에서 네 가지 결정적 정책 가정 중 자기일관인 것은 $(\texttt{Right},\texttt{Left})$ 하나뿐이며,
나머지 셋은 여전히 해를 주되 그것은 그 나쁜 정책의 가치일 뿐이다. 한편 $\max$는 비확장적이므로
$\Tstar$ 역시 $\gamma$-축소 사상이고, 그래서 경우 분석을 건너뛴 반복 대입이 통한다. 예제가 333회에서
...
파일 & 내용 & 대응 절 \\
\midrule
\texttt{ch03/s01\_bellman\_linear.py}    & $(I-\gamma P_\pi)v=r_\pi$의 직접 해                & \S\ref{sec:linear} \\
\texttt{ch03/s02\_bellman\_iterative.py} & 반복 대입과 오차의 지수적 감소                    & \S\ref{sec:rate} \\
\texttt{ch03/s03\_q\_function.py}        & $V\leftrightarrow Q$ 변환과 식~\eqref{eq:q-bellman} 검증 & \S4 \\
\texttt{ch03/s04\_bellman\_optimal.py}   & 벨만 최적 방정식의 반복 대입, 손풀이, 전수 탐색 대조 & \S5 \\
\bottomrule
\end{tabular}
\end{table}
...
표~\ref{tab:policy-map}, 표~\ref{tab:cases}, 그리고 \S\ref{sec:contraction}의 ulp 계산은 본고를 위해
별도로 계산한 값이다.
