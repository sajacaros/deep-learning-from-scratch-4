...
\begin{abstract}
\noindent
앞 장의 몬테카를로법(MC법)은 환경 모델을 버리고 표본 평균으로 가치를 얻었지만, 수익 $G_t$의 정의에
에피소드 끝까지의 보상이 들어 있어 판이 끝나야 한 번 배울 수 있었다. \emph{TD법}(Temporal
Difference)은 기다리지 않는다. 아직 받지 못한 보상의 합을 지금 갖고 있는 추정치
$V(S_{t+1})$로 대신 놓고 한 걸음마다 갱신한다. 본고는 그 교체에서 출발하여 \emph{TD 정책 평가},
\emph{온-정책 제어인 SARSA}, \emph{중요도 샘플링으로 보정하는 오프-정책 SARSA}, 그리고 \emph{Q
러닝}을 정리하고, 네 알고리즘이 ``다음 상태의 값을 누구에게서 받아 오는가''라는 하나의 축 위의
서로 다른 지점임을 보인다. 원서\cite{dlfs4} 6장은 예제 코드를 제공하므로 본고는 그것을 그대로
쓰되, 원서가 관찰로만 남기거나 아예 짚지 않은 자리에 다섯 가지 논의를 더하였다. 절 구성은 원서
6장의 목차를 그대로 따르며, 더한 논의는 별도의 절로 떼지 않고 그것이 필요해지는 자리의 본문에
``[보충]''으로 표시해 두었다.
첫째, 원서가 표로만 적어 둔 ``TD법은 편향이 있고 분산이 작다''를 같은 실험 안에서 두 양으로 갈라
측정하고, 두 방법의 우열이 에피소드 수에 따라 뒤집히는 지점을 보인다.
둘째, 원서가 그림 6-7의 아래 줄 화살표를 ``SARSA가 안전한 길을 배운 것''으로 읽는 대목을 검산한다.
$\varepsilon$-소프트 정책 가운데 가장 좋은 것을 정확히 계산하면 그 자리의 최선은 여전히
\texttt{UP}이고, 화살표가 왼쪽을 가리키는 진짜 원인은 $\alpha=0.8$이다. 에피소드를 $20$배로 늘려도
바뀌지 않지만 $\alpha$를 $0.1$로 낮추면 시드 $20$개 중 $17$개가 뒤집힌다.
셋째, 예제의 오프-정책 SARSA가 쓰는 목표 $\rho(R+\gamma Q')$가 교과서의
$R + \gamma\rho Q'$와 다르다는 것을 밝히고, 이 환경에서는 두 형태의 차이가 실측으로 나타나지 않는
까닭을 갱신 $136{,}813$회 가운데 $23$회라는 숫자로 제시한다.
넷째, Q 러닝의 오차가 $0.00$으로 찍히는 것이 알고리즘이 아니라 환경 덕임을 보인다. 이 환경에서 Q
러닝의 TD 목표는 분산이 정확히 $0$이라 앞 장에서 본 고정값 $\alpha$의 잔차 띠가 사라지며,
전이에 미끄러짐을 넣으면 그 정확도도 함께 사라진다.
다섯째, 샘플 모델 판본이 $1{,}000$ 에피소드에서 남기는 오차가 하필 $1.0000$인 까닭을 밝힌다.
...
받은 보상을 전부 더한 값이므로, 이 한 줄을 실행하려면 판이 끝나야 한다.

값이 두 가지다. 판이 길면 갱신이 그만큼 드물어진다. 앞 장의 그리드 월드는 무작위 정책에서 한
에피소드가 평균 $44.8$걸음이었으니 $45$걸음을 걷고 갱신을 한 묶음 하는 셈이었다. 그리고 끝이 없는
\emph{지속적 과제}에는 아예 쓸 수 없다. $G_t$가 정의되지 않기 때문이다.

이 장은 $G_t$를 기다리지 않는다. 앞 장에서 이미 쓴 재귀 구조
$G_t = R_t + \gamma G_{t+1}$을 다시 꺼내면, 모르는 것은 $G_{t+1}$ 하나뿐이다. 그 자리에 지금 갖고
있는 추정치 $V(S_{t+1})$을 넣는다.
\begin{equation}
  V(S_t) \;\leftarrow\; V(S_t) + \alpha\bigl\{\, R_t + \gamma V(S_{t+1}) - V(S_t) \,\bigr\}.
  \label{eq:td-update-intro}
\end{equation}
$R_t$와 $S_{t+1}$은 한 걸음만 걸으면 알 수 있으므로 갱신이 걸음마다 일어난다. 아직 부정확한
추정치를 목표로 삼아 다른 추정치를 고치는 이 방식을 \emph{부트스트래핑}이라 하며, 앞 장의 DP도 같은
성격이었다.

교체의 대가는 편향이다. $G_t$는 참값 주위에 고르게 흩어지지만 $R_t + \gamma V(S_{t+1})$은 $V$가
틀린 만큼 한쪽으로 치우친 값이다. 대신 얻는 것이 작은 분산과 잦은 갱신이며, 이 저울이 어느 쪽으로
기우는지를 \S\ref{sec:eval} 끝에서 실측한다.

\subsection*{정책이 갱신식에서 사라지는 자리}

평가에 개선을 붙여 제어로 넘어가면 앞 장에서 만난 갈림길이 다시 나온다. 걷는 정책과 평가받는
정책을 같게 둘 것인가 다르게 둘 것인가. 앞 장의 답은 중요도 가중치 $\rho$였고, 그 $\rho$가 걸음마다
곱해 쌓이면서 분산이 커지는 것이 MC법 오프-정책의 약점이었다.

TD법에서는 한 걸음만 내다보므로 곱이 하나로 끝난다. 앞 장의 약점이 크게 줄어드는 것이 이 장의 첫
번째 소득이다. 두 번째 소득은 더 크다. 다음 상태의 값을 ``실제로 고른 행동의 $Q$'' 대신
``$\max_a Q$''로 바꾸면 정책이 갱신식에서 통째로 사라지고, 보정할 것이 없으니 $\rho$도 필요
없어진다. 그 알고리즘이 Q 러닝이며, 3장에서 벨만 방정식과 벨만 최적 방정식을 가르던
$\sum_a \pi(a\mid s)$ 대 $\max_a$의 차이가 여기서 알고리즘의 차이로 나타난다.

\subsection*{본고의 구성과 기여}

본고는 원서 6장의 목차를 그대로 따른다. \S\ref{sec:eval}에서 TD법을 도출해 정책 평가에 쓰고,
\S\ref{sec:sarsa}에서 $Q$로 바꾸어 온-정책 제어인 SARSA를 만든다. \S\ref{sec:offsarsa}에서 걷는
정책과 평가받는 정책을 갈라 한 걸음분 $\rho$로 보정하고, \S\ref{sec:qlearning}에서 그 $\rho$마저
없애는 Q 러닝에 이른다. \S\ref{sec:models}에서 정책을 확률분포로 들고 있을 필요가 없다는 것을
확인해 코드를 $Q$ 하나로 줄인다. \S\ref{sec:summary}에서 정리한다.

원서 6장은 예제 코드를 제공하므로 본고는 그것을 그대로 쓰되, 원서가 관찰로만 남기거나 아예 짚지
않은 다섯 자리에 논의를 더하였다. 절을 따로 만들어 밀어 두지 않고, 그것이 필요해지는 자리의 본문에
``[보충]''으로 표시해 두었다.

\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{편향과 분산을 갈라 보기}(\S\ref{sec:eval} 끝). 원서는 MC법과 TD법의 성질을 표로
        적으며 편향은 TD법에, 분산은 MC법에 있다고 말하지만 어느 쪽이 얼마인지는 적지 않는다. 시드
        $200$개로 두 양을 따로 측정하고, 두 방법의 우열이 에피소드 수에 따라 뒤집히는 것을 보인다.
  \item \textbf{그림 6-7의 화살표는 안전한 길이 아니다}(\S\ref{sec:sarsa} 끝). 원서는 SARSA가
        고른 아래 줄의 왼쪽 화살표를 ``$\varepsilon$만큼 헤매는 자기 자신을 평가하므로 폭탄을
        피한 것''으로 읽는다. $\varepsilon$-소프트 정책 가운데 가치가 가장 높은 것을 정확히
        계산하면 그 자리의 최선은 $\varepsilon=0.5$까지도 \texttt{UP}이며, 화살표의 원인은 위험
        회피가 아니라 $\alpha=0.8$이다. 원인을 에피소드 수, $\alpha$, 탐색 시작 셋으로 갈라
        확인한다.
  \item \textbf{$\rho$를 곱하는 자리}(\S\ref{sec:offsarsa} 끝). 예제의 목표
        $\rho(R + \gamma Q')$는 보상 $R$까지 함께 보정하는데, 교과서의 형태는
        $R + \gamma\rho Q'$로 $\rho$가 다음 상태의 값에만 붙는다. 두 형태를 같은 조건에서 돌려
        견주고, 이 환경에서 차이가 드러나지 않는 까닭을 밝힌다.
  \item \textbf{오차 $0.00$은 환경이 준 것}(\S\ref{sec:qlearning} 끝). 원서는 Q 러닝의 값이 앞
        장 DP의 값과 정확히 일치하는 것을 이 장에서 가장 인상적인 결과로 제시한다. 이 환경에서 Q
        러닝의 TD 목표는 분산이 정확히 $0$이라 앞 장 [보충]에서 본 폭
        $\sqrt{\alpha/(2-\alpha)}\,\sigma$의 잔차 띠가 사라진다는 것을 보이고, 전이에 미끄러짐을
        넣어 정확도가 함께 사라지는 것을 확인한다.
  \item \textbf{하필 $1.0000$인 오차}(\S\ref{sec:models} 끝). 원서 예제는 에피소드 수가 $1{,}000$
        이라 학습이 덜 된 상태로 끝나는데, 남는 오차가 어중간한 값이 아니라 정확히 $1.0000$이다.
        그 값이 어느 칸에서 오는지 밝힌다.
\end{enumerate}
...
\begin{equation}
  ...
  \label{eq:bellman-sum}
\end{equation}
이고, 등호를 갱신식으로 바꿔 읽은 것이 앞 장의 반복적 정책 평가였다. 우변에 $\pi$와 $p$가 그대로
남아 있어 모델을 알아야만 계산된다.

펼치지 않고 식~\eqref{eq:bellman-exp}의 기댓값 모양 그대로 두면 사정이 다르다. 우변에 $p$도
...
\begin{table}[htbp]
\centering
\caption{TD 정책 평가의 오차(무작위 정책, $\alpha=0.01$, 시드 $50$개). 기준은 앞 장의 정확한
$v_\pi$다. 같은 양을 MC법과 짝지어 견준 것은 이 절 끝의 [보충]에 있다.}
\label{tab:td-eval}
\small
\begin{tabular}{@{}lr@{}}
\toprule
지표 & 값 \\
\midrule
$\max_s |V(s) - v_\pi(s)|$ 의 평균 & $0.0855$ \\
$V(s)$ 의 RMSE                    & $0.0376$ \\
$V(s) - v_\pi(s)$ 의 평균(부호 포함) & $-0.0011$ \\
\bottomrule
\end{tabular}
\end{table}

$\alpha$를 $0.01$로 작게 잡은 것은 갱신 횟수가 많기 때문이다. 한 에피소드에 한 번씩 갱신하던 앞
장과 달리 걸음마다 갱신하므로, $1{,}000$ 에피소드에서 갱신이 수만 번 일어난다. $\alpha$를 크게
잡으면 앞 장 [보충]에서 본 잔차 띠가 그만큼 넓어져 값이 출렁인다.

\subsubsection{[보충] 편향과 분산을 갈라 보기}
\label{sec:bias-var}

원서는 편향이 TD법에 있고 분산이 MC법에 있다고 적을 뿐 어느 쪽이 얼마인지는 적지 않는다. 두
양을 갈라 보려면 따로 뽑아야 한다. 같은 시드로 만든 궤적 하나를 세 갱신식에 똑같이 먹이고 시드 $60$개에서
상태별로 평균과 표준편차를 구했다. 평균이 참값에서 벗어난 만큼이 편향이고, 시드 사이의 표준편차가
분산 쪽이다.
...
$\alpha$를 맞춰 놓고 견주면 원서의 표가 그대로 확인된다. $1{,}000$ 에피소드에서 TD법의 표준편차는
$0.0295$이고 MC법은 $0.0924$로 세 배가 넘게 벌어지며, RMSE도 $0.0358$ 대 $0.0971$로 그 차이를
그대로 따라간다. TD 목표에는 한 걸음분의 무작위성만 들어 있고 $G_t$에는 평균 $44.8$걸음분이 쌓인다는
사실이 숫자로 나온 것이다.

편향은 초반에만 보인다. $250$ 에피소드에서 TD법의 편향이 $0.0749$로 MC법의 $0.0154$보다 다섯 배
가까이 큰데, $V$가 아직 초깃값 $0$ 근처에 있어 TD 목표 $R_t + \gamma V(S_{t+1})$이 통째로 작게
나오기 때문이다. $1{,}000$ 에피소드에서 $0.0044$까지 떨어져 MC법과 같은 수준이 된다. 부트스트래핑의
편향은 $V$가 채워지면서 스스로 걷힌다.

한 가지가 더 보인다. TD법의 RMSE가 $1{,}000$ 에피소드부터 $0.036$ 근처에서 더 내려가지 않는다.
앞 장 [보충]에서 본 고정값 $\alpha$의 잔차 띠이며, MC법도 $\alpha=0.01$을 쓰면 $0.098$에서 똑같이
멈춘다. 반면 $1/n$ 평균은 $8{,}000$ 에피소드까지 계속 내려가 $0.0094$에 이른다. 그래서 예제가 쓰는
...
것이며, 앞 장에서 본 저울이 여기서도 같은 방향으로 기운다.

정리하면 원서가 표로 적은 두 성질은 둘 다 사실이지만 유효 기간이 다르다. 분산의 차이는 학습이
끝날 때까지 남고, 편향의 차이는 초반 몇백 에피소드에서만 보인다.
...
\begin{equation}
  ...
  \label{eq:eps-greedy}
\end{equation}
SARSA는 식~\eqref{eq:sarsa}의 평가와 식~\eqref{eq:eps-greedy}의 개선을 걸음마다 번갈아 한다. 앞
장의 MC 제어가 에피소드마다 번갈아 하던 것을 한 걸음 단위로 잘게 쪼갠 것이며, 앞 장에서
\emph{일반화된 정책 반복}이라 부른 틀 안에서 더 극단으로 간 지점이다.
...
\lstinline|deque(maxlen=2)|는 새 데이터를 넣으면 가장 오래된 것을 자동으로 밀어낸다.

목표에 도달했을 때 \lstinline|agent.update(next_state, None, None, None)|을 한 번 더 부르는 것도
길이 $2$라는 구조에서 나온다. 마지막 걸음은 \lstinline|memory|에 하나만 들어 있어 갱신되지 못하므로,
더미를 밀어 넣어 마지막 $(S,A)$까지 갱신되게 한다. \lstinline|done=True|라 \lstinline|next_q|는
$0$이 된다.
...
\begin{table}[htbp]
\centering
\caption{SARSA를 시드 $50$개로 돌린 성적($\alpha=0.8$, $\varepsilon=0.1$, $10{,}000$ 에피소드,
기준은 앞 장이 쓴 최적해). 앞 장의 온-정책 MC 제어가 같은 조건에서 $9.20/10$과 RMSE $0.1439$
였다.}
\label{tab:sarsa}
\small
\begin{tabular}{@{}lr@{}}
...
최적 행동 일치(평균)                       & $7.62/10$ \\
$10$칸 모두 맞힌 시드의 비율               & $0\%$ \\
$\max_s |\max_a Q(s,a) - v_*(s)|$ 의 평균  & $0.5709$ \\
$\max_a Q(s,a)$ 의 RMSE                    & $0.2475$ \\
$\max_a Q(s,a) - v_*(s)$ 의 평균(부호 포함)& $-0.1409$ \\
\bottomrule
\end{tabular}
\end{table}

앞 장의 MC 제어보다 나쁘다. $10$칸을 모두 맞힌 시드가 하나도 없고 값의 오차도 두 배 가까이 크다.
한 걸음마다 갱신하니 더 나아야 할 것 같은데 그렇지 않다.

원인의 일부는 평가 대상에 있다. 앞 장에서 본 대로 온-정책 제어가 다가가는 값은 $v_*$가 아니라
$\varepsilon$-탐욕 정책 자신의 가치이므로 $v_*$보다 아래다. 그러나 그 몫은
$\varepsilon=0.1$에서 평균 $-0.0235$였고, 여기서 관찰된 $-0.1409$의 $6$분의 $1$에 지나지 않는다.
나머지는 $\alpha$에서 온다.
...
여야 한다. SARSA가 수렴하는 대상은 $\varepsilon$-소프트 정책 가운데 가치가 가장 높은 것이므로,
앞 장의 정책 반복법을 $\varepsilon$-소프트 정책으로 제한해 돌리면 그 값을 표본 없이 얻을 수 있다.
...
예제가 쓰는 $\varepsilon=0.1$에서 $(2,2)$의 최선은 \texttt{UP}이고 \texttt{LEFT}보다 $0.16$ 앞선다.
$\varepsilon$을 $0.5$까지 올려도 순서가 바뀌지 않는다. 위험 회피가 실제로 정책을 뒤집으려면
$\varepsilon$이 $0.9$는 되어야 한다.
...
그러면 화살표는 어디서 오는가. 원인을 하나씩 없애 보면 갈린다.
...
앞 장 [보충]에서 고정값 $\alpha$가 추정치를 한 점에 모으지 못하고 폭
$\sqrt{\alpha/(2-\alpha)}\,\sigma$의 띠 안에 남긴다는 것을 보았다. $\alpha=0.8$이면 계수가
$\sqrt{0.8/1.2} = 0.8165$로, 앞 장이 쓰던 $\alpha=0.1$의 $0.2294$보다 세 배 넘게 크다. 남은 것은
$\sigma$, 곧 TD 목표가 얼마나 흔들리는지다.
...
$10{,}000$ 에피소드 이후의 값을 모아 실제로 재 보면 표준편차가 $0.2381$로 예상보다 $2$배 가까이
넓다. 앞 장 [보충]에서도 측정값이 이론값의 $1.5$배 남짓으로 나왔는데, 이론이 표본들의 독립을
가정하는 데 비해 실제로는 목표값 자신이 학습 중에 계속 움직이기 때문이다. $\alpha$를 $0.1$로
낮추면 같은 값이 $0.0691$로 좁아지고, Q 러닝에서는 $0.0000$이다.

정리하면 그림~\ref{fig:sarsa-result}의 왼쪽 화살표는 위험을 피한 결과가 아니라 $\alpha=0.8$이
남긴 폭 $0.2$ 남짓의 흔들림에 $(2,2)$의 두 행동 차이 $0.16$이 묻혀 버린 결과다. 그리고 그 흔들림은
스스로를 키운다. 값이 낮게 찍힌 칸은 정책이 등을 돌리고, 등을 돌리면 덜 방문되며, 덜 방문되면
값이 그대로 남는다. 방문 횟수가 시작 칸 $14{,}271$번 대 $(2,2)$ $378$번으로 $38$배 벌어지는 것이
그 고리의 결과다. $\alpha$를 낮추면 $(2,2)$ 방문이 $6{,}089$번으로 $16$배 늘어난다.

원서가 $\alpha=0.8$을 고른 데는 이유가 있다. \S\ref{sec:qlearning}의 Q 러닝에서는 이 값이
문제를 일으키지 않으며, 오히려 빠른 학습에 도움이 된다. 같은 $\alpha$가 두 알고리즘에서 갈리는
까닭은 Q 러닝의 TD 목표에 분산이 없다는 데 있다.
...
대해 하고 싶다. 그렇게 하면 표~\ref{tab:eps-soft}에서 본 $\varepsilon$ 몫이 사라진다.

먼저 갱신식에서 확률이 개입하는 자리를 찾는다.
...
$A_{t+1} \sim \pi$로 뽑혔다면 지금까지의 온-정책 SARSA이고,
\begin{equation}
  Q(S_t,A_t) \;\leftarrow\; Q(S_t,A_t)
    + \alpha\bigl\{\, R_t + \gamma Q(S_{t+1},A_{t+1}) - Q(S_t,A_t) \,\bigr\},
  \qquad A_{t+1} \sim \pi,
...
\begin{table}[htbp]
\centering
\caption{온-정책과 오프-정책 SARSA를 시드 $50$개로 견준 것($\alpha=0.8$, $\varepsilon=0.1$,
$10{,}000$ 에피소드).}
\label{tab:on-vs-off}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
 & 최적 행동 & $10/10$ & $\max_s|\max_a Q - v_*|$ & RMSE \\
\midrule
SARSA (\texttt{s02})            & $7.62/10$ & $0\%$  & $0.5709$ & $0.2475$ \\
오프-정책 SARSA (\texttt{s03})  & $8.36/10$ & $22\%$ & $0.6199$ & $0.2885$ \\
\bottomrule
\end{tabular}
\end{table}

앞 장 5.5절에서 본 것과 같은 모양이 나온다. 정책은 더 잘 맞히고($7.62 \to 8.36$칸,
...
$\rho=0$이 되는 것은 갱신의 $6.99\%$뿐이다. 앞 장 MC법에서 같은 값이 $59.9\%$였던 것과 크게
다른데, 앞 장에서는 누적곱이라 한 걸음만 어긋나도 그보다 앞선 걸음이 전부 죽었기 때문이다. 한
걸음짜리 $\rho$가 $0$이 되려면 $A_{t+1}$이 탐욕 행동이 아니어야 하고, $\varepsilon$-탐욕 정책에서
그 확률은 $\varepsilon \cdot 3/4 = 7.5\%$이고, 실측한 $6.99\%$가 거기에 맞는다.

두 형태가 실제로 갈리는 것은 $\rho=0$이면서 보상이 $0$이 아닌 갱신뿐인데, 그런 갱신이 $136{,}813$회
가운데 $23$회다. 이 환경은 보상이 목표 칸과 폭탄 칸에서만 나오고 나머지 $10$칸에서는 모두 $0$이라
두 형태가 만날 기회 자체가 드물다. 보상이 걸음마다 나오는 환경이라면 두 형태의 차이가 드러날
것이며, 그때는 교과서 형태를 써야 한다.
...
방정식의 표본 판본이라 부른다. 문제도 거기서 나온다.
$\pi(a\mid s)$에서 뽑아야 하는데 실제로 걷는 것은 $b$다. 오프-정책으로 만들려면 $\rho$로 그
어긋남을 메워야 했다.
...
\begin{equation}
  ...
  \label{eq:qlearning}
\end{equation}
SARSA의 식~\eqref{eq:sarsa}과 다른 곳은 $Q(S_{t+1},A_{t+1})$이 $\max_a Q(S_{t+1},a)$로 바뀐 자리
하나다.
...
\begin{table}[htbp]
\centering
\caption{이 장의 네 알고리즘을 같은 조건에서 견준 것($\varepsilon=0.1$, $10{,}000$ 에피소드,
시드 $50$개). 위의 셋은 $\alpha=0.8$이고 정책 평가만 $\alpha=0.01$이라 따로 두었다.}
\label{tab:all}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
SARSA (\texttt{s02})                     & $7.62/10$ & $0\%$  & $0.5709$ & $0.2475$ \\
오프-정책 SARSA (\texttt{s03})           & $8.36/10$ & $22\%$ & $0.6199$ & $0.2885$ \\
Q 러닝 (\texttt{s04})                    & $9.66/10$ & $86\%$ & $0.0674$ & $0.0245$ \\
Q 러닝, 샘플 모델 (\texttt{s05}, $10{,}000$회) & $9.84/10$ & $84\%$ & $0.1132$ & $0.0361$ \\
\midrule
\multicolumn{5}{@{}l}{\footnotesize 참고: 앞 장의 온-정책 MC 제어는 $9.20/10$, $48\%$, $0.3359$, $0.1439$였다.} \\
\bottomrule
\end{tabular}
\end{table}

차이가 크다. Q 러닝은 $10$칸을 모두 맞히는 시드가 $86\%$이고 값의 RMSE가 SARSA의 $10$분의 $1$이다.
시드 $50$개 가운데 $43$개에서 $\max_a Q$가 앞 장 DP의 $v_*$와 \emph{완전히} 같았다. 모델을 보지
않고 걸어 다니기만 해서 얻은 값이 모델을 보고 계산한 값과 소수점 아래까지 일치하는 것이다.
...
$\varepsilon$-소프트 최적 가치이지만 Q 러닝이 다가가는 곳은 $q_*$ 자체다. 식~\eqref{eq:qlearning}이
벨만 최적 방정식의 표본 판본이기 때문이다.

\subsubsection{[보충] 오차 \texorpdfstring{$0.00$}{0.00}은 환경이 준 것}
\label{sec:exact}

값이 정확히 맞는 것이 알고리즘의 일반적인 성질은 아니다. $\alpha=0.8$이라는 큰 값을 쓰면서도
흔들리지 않는다는 사실 자체가 앞 장 [보충]과 어긋나 보인다. 고정값 $\alpha$는 추정치를 폭
...
$\alpha=0.8$이 오히려 이득이 된다. 표~\ref{tab:target-sigma}의 두 열이 갈리는 자리가 바로 여기다.

설명이 맞다면 환경을 확률적으로 바꾸었을 때 정확도가 사라져야 한다. 고른 행동이 확률
\emph{미끄러짐}으로 엉뚱한 행동으로 바뀌게 해 두고 같은 실험을 반복하였다.
...
미끄러짐을 $10\%$만 넣어도 정확히 맞는 시드가 하나도 남지 않는다. 그리드 월드의 전이가 결정적이라는
성질이 준 결과이며, 확률적 환경에서 같은 정확도를 원하면 $\alpha$를 줄이거나 시간에 따라 줄여야
한다.
...
때뿐이고, 그 규칙은 $\varepsilon$-탐욕 그대로다. 확률분포를 저장할 이유가 없다.

분포를 들고 있는 것이 불리해지는 경우도 있다. 행동이 많으면 딕셔너리가 그만큼 커지고, 행동이
연속값이면 표로 적는 것 자체가 불가능하다.
...
모양이 되었다. 알고리즘은 바뀌지 않았고 표현만 바뀌었다.

표~\ref{tab:all}에서 두 판본의 성적이 사실상 같게 나온다. 이 형태가 다음 장부터
신경망으로 넘어갈 때의 출발점이 된다. 신경망은 $Q$만 근사하면 되고 정책은 규칙으로 두면 되기
때문이다.
...
상태로 끝난다. 남는 오차가 눈에 띄는 것은 크기가 아니라 값이다.
...
$1.0000$이라는 값은 $v_*(1,3) = 1.0000$에서 그대로 온다. $(1,3)$은 폭탄 칸이고 목표 바로 아래에
있어 한 걸음이면 $+1$을 받으므로 최적 가치가 정확히 $1$이다. 그 칸에 한 번도 서 보지 못하면
$Q((1,3),\cdot)$이 초깃값 $0$에 그대로 남고, 오차가 $1.0000 - 0 = 1.0000$으로 찍힌다. 정확히
$1.0000$인 시드가 $24$개, $(1,3)$을 한 번도 밟지 않은 시드가 $24$개로 수가 같다.

같은 사정으로 $0.7290$도 나온다. $(2,3)$을 방문하지 않으면 오차가 $v_*(2,3) = 0.7290$이 된다.
$1{,}000$ 에피소드에서 $41/50$ 시드가 그 칸을 밟지 못한다. 두 칸이 목표에서 가장 먼 구석이고
탐욕 정책이 지나갈 이유가 없어서다.

오차가 어중간한 소수가 아니라 참값과 딱 떨어지게 나오면 학습이 부정확한 것이 아니라 \emph{아예
시작되지 않은} 칸이 있다는 신호다. 알고리즘의 문제가 아니라 경험이 닿지 않은 곳의 문제이며,
에피소드를 $10{,}000$으로 늘리면 방문하지 않은 칸이 사라지고 그런 오차도 함께 사라진다.
...
$\varepsilon=0.1$, $\alpha=0.8$)에서 이 장이 측정한 것이며, 정책 평가는 제어가 아니라 성적을
적지 않았다.}
\label{tab:summary}
\small
\begin{tabular}{@{}lllr@{}}
\toprule
 & 목표값 & 수렴 대상 & $\max_a Q$ 의 RMSE \\
\midrule
TD 정책 평가    & $R_t + \gamma V(S_{t+1})$              & $v_\pi$                       & --- \\
SARSA           & $R_t + \gamma Q(S_{t+1},A_{t+1})$      & $\varepsilon$-소프트 최적 가치 & $0.2475$ \\
오프-정책 SARSA & $\rho\,(R_t + \gamma Q(S_{t+1},A_{t+1}))$ & 탐욕 정책의 $q$            & $0.2885$ \\
Q 러닝          & $R_t + \gamma \max_a Q(S_{t+1},a)$     & $q_*$                         & $0.0245$ \\
\bottomrule
\end{tabular}
\end{table}
...
대신한다. 표현이 바뀌어도 갱신식은 이 장의 식~\eqref{eq:qlearning} 그대로이며, 바뀌는 것은
$Q(S_t,A_t)$를 어떻게 저장하고 어떻게 고치느냐다.
...
\emph{예제 코드의 실측}(표~\ref{tab:td-eval}, 표~\ref{tab:sarsa}, 표~\ref{tab:sarsa-cause},
표~\ref{tab:on-vs-off}, 표~\ref{tab:target-form}, 표~\ref{tab:rho-dist}, 표~\ref{tab:all},
표~\ref{tab:slip}, 표~\ref{tab:untouched}, 표~\ref{tab:summary})은 \texttt{ch06/}의 예제를
그대로 돌리고 계측만 덧붙인 것이다. 시드는 \lstinline|np.random.seed()|에 $0$부터 차례로 넣었고, 항목 대부분이 $50$개다.
원인을 갈라 보는 표~\ref{tab:sarsa-cause}에는 에피소드를 $200{,}000$까지 늘리는 설정이 있어
계산 시간 때문에 시드가 $20$개이고, 방문 분포와 칸별 오차도 같은 이유로 $20$개다. $\rho$의 분포는 시드 $0$ 하나를
계측한 것이다.
