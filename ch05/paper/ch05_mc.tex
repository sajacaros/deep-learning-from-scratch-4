...
논의는 별도의 절로 떼지 않고 그것이 필요해지는 자리의 본문에 ``[보충]''으로 표시해 두었다.
첫째, ``몬테카를로''라는 이름의 유래와 기법으로서의 정의를 밝히고, 강화 학습이 이 말을
``완결된 수익의 평균''이라는 좁은 뜻으로 쓴다는 것을 적는다.
둘째, 예제 코드가 말없이 택한 \emph{모든 방문} 방식과 교과서의 \emph{첫 방문} 방식이 어떻게 다른지를
밝히고, 이 환경에서 한 에피소드가 평균 $44.8$걸음이라 상태마다 평균 $4.5$번씩 다시 지난다는 사실
때문에 모든 방문이 실측에서 앞선다는 것을 보인다.
셋째, 표본 오차가 $1/\sqrt{n}$로만 줄어든다는 것을 에피소드 수와 상태별 방문 횟수 양쪽에서 실측하고,
앞 장의 DP가 $1104$번의 백업으로 얻은 정확도에 MC법이 이르려면 약 $1.5\times10^{7}$걸음이 필요함을
환산해 보인다.
넷째, 고정값 $\alpha$가 남기는 잔차의 표준편차가 $\sqrt{\alpha/(2-\alpha)}\,\sigma_G$라는 바닥에서
멈춘다는 것을 실측과 이론값으로 나란히 확인하고, 그럼에도 제어에서는 $1/n$ 평균보다 나은 이유를
같은 실험 안에서 보인다.
다섯째, 오프-정책 예제의 대상 정책이 \emph{결정적}이라 중요도 가중치가 $0$ 아니면 큰 값으로만
나타난다는 것을 밝히고, 갱신의 절반 이상($53.1$--$65.5\%$)에서 $\rho$가 이미 $0$이라는 실측과 동점 처리에서 비롯한
$40$배 가중치의 존재를 제시하여 오프-정책이 ``정책은 더 잘 맞히면서 값은 더 부정확한'' 까닭을
설명한다.
...
이 장이 바꾸는 것은 식~\eqref{eq:dp-update}의 $\sum$ 하나다. 상태 가치 함수의 정의가 애초에
기댓값이라는 데서 출발한다.
\begin{equation}
...
배운다. 이 셋이 각각 \S\ref{sec:impl} 끝, \S\ref{sec:control}, \S\ref{sec:offpolicy}에서 문제가
되는 자리를 만든다.

\subsection*{본고의 구성과 기여}

본고는 원서 5장의 목차를 그대로 따른다. \S\ref{sec:basic}에서 주사위 예제로 몬테카를로법의 뼈대와
증분 평균을 세우고, \S\ref{sec:eval}에서 그것을 정책 평가에 옮기며 한 에피소드에서 모든 상태의 수익을
뽑아내는 뒤에서부터 계산하기를 얻는다. \S\ref{sec:impl}에서 $3\times4$ 그리드 월드에 구현하고,
\S\ref{sec:control}에서 평가에 개선을 붙여 제어로 넘어가며 $\varepsilon$-탐욕 정책과 고정값 $\alpha$
두 처방을 붙인다. \S\ref{sec:offpolicy}에서 행동하는 정책과 평가할 정책을 갈라 중요도 샘플링으로
잇고, \S\ref{sec:summary}이 정리다.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{몬테카를로라는 이름}(\S\ref{sec:basic} 끝). 원서는 이 이름을 풀지 않고 쓴다. 울람과
        메트로폴리스가 이 이름을 고른 사정을 밝히고, 기법으로서의 정의가 ``난수를 써서 수치 답을 얻는
        방법''이라는 넓은 뜻임을 적은 뒤, 강화 학습이 이 말을 ``완결된 수익의 평균''이라는 훨씬 좁은
        뜻으로 쓴다는 것을 구별한다. 앞 장의 [보충] ``왜 동적 프로그래밍인가''와 짝을 이룬다.
  \item \textbf{첫 방문과 모든 방문}(\S\ref{sec:eval} 끝). 한 에피소드에서 같은 상태가 여러 번
        나타날 때 그 전부를 표본으로 삼을지 처음 하나만 삼을지는 정해야 하는 선택인데, 원서는 이
        선택이 있다는 것조차 말하지 않고 \emph{모든 방문} 쪽을 쓴다. 두 방식의 통계적 성질이 어떻게
        다른지 적고, 이 환경에서 어느 쪽이 실제로 정확한지를 실측으로 가른다.
  \item \textbf{표본 오차는 얼마나 느리게 줄어드는가}(\S\ref{sec:impl} 끝). 원서는 MC법의 표와 DP의
        표가 ``비슷하지만 똑같지는 않다''는 관찰에서 멈춘다. 오차가 에피소드 수와 상태별 방문 횟수
        양쪽에 대해 $1/\sqrt{n}$로 줄어듦을 실측으로 확인하고, 앞 장의 DP가 치른 비용과 같은 단위로
        환산하여 견준다.
  \item \textbf{고정값 $\alpha$가 남기는 바닥}(\S\ref{sec:control} 끝). 원서는 정책이 변하므로
        $1/n$ 평균 대신 고정값 $\alpha$를 쓴다고만 적는다. $\alpha$를 쓰면 추정치가 한 점으로
        모이지 않고 폭 $\sqrt{\alpha/(2-\alpha)}\,\sigma_G$의 띠 안에서 계속 흔들린다는 것을
        이론값과 실측으로 나란히 보이고, 그럼에도 제어에서는 그편이 나은 이유를 같은 실험 안에서
        확인한다.
  \item \textbf{$\rho$는 0 아니면 폭발}(\S\ref{sec:offpolicy} 끝). 원서는 중요도 샘플링의 분산이
        커진다는 것을 일반론으로 적고 넘어간다. 예제의 대상 정책이 \emph{결정적}이라는 사실 때문에
        이 환경에서 가중치가 취하는 값이 사실상 셋뿐임을 밝히고, 갱신의 절반 이상에서 $\rho$가 이미
        $0$이라는 것과 동점 처리에서 $40$배 가중치가 생긴다는 것을 실측으로 제시한다.
\end{enumerate}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\linewidth]{{fig 5-3}.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\linewidth]{{fig 5-4}.png}
...
$1{,}000$번 시행하면 $6.92$ 언저리가 나온다. 참값 $7$에 가깝지만 정확히 $7$은 아니고, 실행할 때마다
...
폭 안에 있다. 식~\eqref{eq:se}에서 이 장 전체를 관통하는 사실 하나가 따라 나온다. \emph{오차를 열 배
줄이려면 시행을 백 배 늘려야 한다.} 이 느림이 \S\ref{sec:impl} 끝의 [보충]에서 다시 문제가 된다.
...
주사위에서 한 일을 그대로 옮긴다. 식~\eqref{eq:def-v}이 말하듯 $v_\pi(s)$는 기댓값이므로 표본 평균으로
...
\begin{equation}
  ...
  \label{eq:mc-avg}
\end{equation}
여기서 짚어야 할 것은 식~\eqref{eq:mc-avg}에 $p$도 $r$도 나오지 않는다는 사실이다. 필요한 것은
행동을 넣으면 보상과 다음 상태를 돌려주는 샘플 모델뿐이다. 앞 장과 이 장을 가르는 선이 바로 여기다.

\subsection{모든 상태의 가치 함수 구하기}
\label{sec:all-states}

식~\eqref{eq:mc-avg}이 주는 것은 상태 \emph{하나}의 가치다. 우리가 원하는 것은 모든 상태의 가치다.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\linewidth]{{fig 5-7}.png}
...
식~\eqref{eq:backward}으로 얻은 것은 $A$, $B$, $C$ 각각의 수익 표본 하나씩이다. 한 판으로 세 상태를
동시에 갱신할 수 있게 되었고, 그림~\ref{fig:naive}의 세 덩어리가 하나로 합쳐졌다.

\subsubsection{[보충] 첫 방문과 모든 방문}
\label{sec:first-every}

식~\eqref{eq:backward}은 한 판에서 $A$, $B$, $C$의 수익을 하나씩 뽑았다. 이 셋이 서로 다른 상태였기
때문에 문제가 없어 보였지만, 같은 상태를 한 판 안에서 두 번 지난다면 이야기가 달라진다. 그때 얻는
수익 표본은 몇 개인가.
...
선택은 둘이다. 처음 지날 때의 수익 하나만 표본으로 삼는 \emph{첫 방문}(first-visit) 방식과, 지날
때마다 하나씩 삼는 \emph{모든 방문}(every-visit) 방식이다. 코드~\ref{lst:mc-eval}의
\lstinline|eval()|은 \lstinline|memory|를 처음부터 끝까지 훑으며 매번 \lstinline|cnts[state] += 1|을
하므로 모든 방문 방식이다. 원서는 이 선택이 있다는 것을 언급하지 않는다.

이 구별은 이 환경에서 사소하지 않다. 무작위 정책의 한 에피소드는 평균 $44.5$걸음인데 학습 대상
상태는 $10$개뿐이므로, 상태 하나를 한 판에 평균 네다섯 번씩 다시 지난다. 재방문이 예외가 아니라
표준이다.
...
\emph{에피소드 수}다. 이 사실이 \S\ref{sec:mc-run} 끝의 [보충]에서 다시 쓰인다.
...
\S\ref{sec:backward}에서 얻은 세 가지가 코드에서 세 줄로 나타난다.
...
\lstinline|reversed(...)|가 식~\eqref{eq:backward}이고, \lstinline|G = gamma * G + reward|가 3장의
재귀 구조이며, 마지막 줄이 \S\ref{sec:dice-impl}의 증분 평균이다. 새로 만든 것은 하나도 없고 앞에서
세운 것들을 이어 붙였을 뿐이다.
...
배운다. 이 차이가 \S\ref{sec:control}에서 훨씬 심각한 문제가 된다.
...
식~\eqref{eq:se}이 예고한 $1/\sqrt{n}$이 실제로 관찰되는지는 앞 절 표~\ref{tab:first-every}의 모든
방문 열이 이미 답한다. 에피소드를 네 배씩 늘릴 때마다 RMSE가 $0.02291 \to 0.01159 \to 0.00560 \to
0.00277 \to 0.00140$으로 거의 정확히 절반씩 줄고, $\log$-$\log$ 최소제곱 기울기가 $-0.507$이다.
최대 오차 쪽도 $-0.518$이다. 지수는 이론값 $-0.5$이며 실측이 유효숫자 두 자리까지 맞는다.

여기서 나오는 결론은 앞에서 적은 것 그대로다. 자릿수 하나를 얻으려면 에피소드를 백 배 늘려야 한다.
표~\ref{tab:first-every}의 마지막 행이 $256{,}000$ 에피소드, 곧 $1{,}100$만 걸음을 걸어 겨우
$0.00275$에 이른 것이 그 값이다.

\paragraph{상태별로.}
\S\ref{sec:mc-run}에서 남겨 둔 물음, 곧 무엇이 어느 칸의 오차를 크게 만드는가에 답할 차례다.
원서는 ``목표에 가까울수록 잘 맞는다''고 적었는데 표~\ref{tab:sigma}가 그렇지 않음을 보인다.
...
$d$로는 설명되지 않으며 오히려 반대 방향이다. 설명하는 것은 $\sigma_G$다. 폭탄 근처의 칸에서
출발하면 어떤 판은 폭탄을 밟아 $-1$을 받고 어떤 판은 피해 가므로 수익이 크게 흩어진다. 반대로
$(2,0)$처럼 폭탄에서 먼 칸은 무작위로 걸어도 결과가 고만고만하다. 표본 평균의 오차를 정하는 것은
평균의 위치가 아니라 표본의 흩어짐이라는 통계의 기본이 그대로 나타난 것이다.

앞 절에서 얻은 ``효과적인 표본 수는 방문 횟수가 아니라 에피소드 수''를 여기 붙이면 상태별 오차의
크기를 예측하는 식이 완성된다.
\begin{equation}
  \mathrm{RMSE}\bigl(V(s)\bigr) \;\approx\; \frac{\sigma_G(s)}{\sqrt{m(s)}},
  \qquad m(s)=\text{$s$를 지난 에피소드의 수}
  \label{eq:per-state-err}
\end{equation}
표~\ref{tab:eff-n}에서 이 예측이 첫 방문 방식과 평균 $0.94$배, 모든 방문 방식과 $0.85$배로 맞는다.
표~\ref{tab:mc-vs-dp}의 한 판 실행에서 $(1,2)$의 차이가 가장 컸던 것도, $(1,3)$이 방문이 가장 적은데도
잘 맞았던 것도 우연이며, 시드를 $300$개 모아 평균하면 식~\eqref{eq:per-state-err}의 순서로 정렬된다.
...
MC법이 같은 오차에 이르려면 얼마가 드는가. 표~\ref{tab:first-every}의 최대 오차 열을 적합하면
...
$22$회를 더 쓰면 되지만 MC법은 지금까지 쓴 것의 백 배를 더 써야 한다. 그림~\ref{fig:mc-error}
왼쪽이 그 대비다.
...
\begin{align}
  \mu(s) &= \arg\max_{a} Q(s,a), \label{eq:greedy-q}\\
  \mu(s) &= \arg\max_{a} \sum_{s'} p(s'\mid s,a)\bigl\{\,r(s,a,s') + \gamma V(s')\,\bigr\}.
  \label{eq:greedy-v}
\end{align}
앞 장이 쓴 것은 식~\eqref{eq:greedy-v}이었다. 그런데 그 식에는 $p$와 $r$가 들어 있다. 모델을 모르는
이 장에서는 쓸 수 없다.

남는 것은 식~\eqref{eq:greedy-q}뿐이고, 그 대가로 평가의 대상이 바뀐다. $V$가 아니라 $Q$를 추정해야
...
식~\eqref{eq:q-update}과 식~\eqref{eq:greedy-q}을 그대로 번갈아 쓰면 문제가 생긴다.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.66\linewidth]{{fig 5-14}.png}
...
탐욕 정책은 결정적이므로 시작 상태에서 출발하면 언제나 같은 길을 걷는다. \S\ref{sec:mc-run}에서
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.52\linewidth]{{fig 5-15}.png}
...
\S\ref{sec:offpolicy}에서 대상 정책과 행동 정책을 가를 때 이 성질을 쓴다.

\lstinline|argmax()|가 동점일 때 무작위로 고른다는 점은 지금은 사소해 보이지만
\S\ref{sec:offpolicy} 끝의 [보충]에서 다시 문제가 된다. 참고로 \texttt{s03\_mc\_control.py}는
...
두 번째 문제는 평균을 내는 방식에 있다. 식~\eqref{eq:q-update}의 $1/n$ 평균은 $n$개의 표본에 모두
같은 비중을 준다. 그런데 제어에서는 표본을 만들어 낸 정책이 계속 바뀐다. $G^{(1)}$은 아직 아무것도
모르는 초기 정책이 낸 수익인데, $10{,}000$번째 수익과 발언권이 같다.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\linewidth]{{fig 5-16}.png}
...
곧 평가를 끝까지 돌리지 않고 곧바로 개선으로 넘어가는 그 구조다. 정책 반복법과 가치 반복법 사이
...
$50$개 시드 $\times$ $10$칸 $=500$개 값 전부에서 그렇다. 이 치우침은 표본 잡음이 아니며, 왜 그런지가
\S\ref{sec:offpolicy}의 출발점이 된다.
...
$1/n$ 평균을 식~\eqref{eq:ema}의 고정값 $\alpha$로 바꾼 것은 공짜가 아니다. 무엇을 내주었는지 먼저
평가 문제에서 확인한다. 제어를 잠시 밀어 두고 \S\ref{sec:mc-run}의 무작위 정책 평가를 두 방식으로
$100{,}000$ 에피소드씩 돌린다. 평가할 정책이 고정되어 있으므로 $\alpha$를 쓸 이유가 전혀 없는
상황이며, 그래서 차이가 순수하게 드러난다.

\begin{table}[htbp]
\centering
\caption{$1/n$ 평균과 고정값 $\alpha$의 잔차($3\times4$ 그리드, 무작위 정책, $100{,}000$ 에피소드,
시드 $20$개). ``측정 표준편차''는 시드마다 얻은 $V(s)$의 시드 간 표준편차를 $10$개 상태에 대해
평균한 것이다. ``이론''은 $\alpha$ 행에서는 아래 식~\eqref{eq:ema-var}이 예측하는
$\sqrt{\alpha/(2-\alpha)}\,\sigma_G$이고, $1/n$ 행에서는 앞 절 [보충]의
식~\eqref{eq:per-state-err}이 예측하는 $\sigma_G/\sqrt{m}$이다.}
\label{tab:alpha}
\small
\begin{tabular}{@{}lllll@{}}
\toprule
...
$1/n$ 평균은 $0.00206$까지 내려가는데 $\alpha=0.1$은 $0.237$에서 멈춘다. $115$배 차이다. 이 환경의
가치가 $-0.79$에서 $+0.21$ 사이에 있다는 것을 생각하면 $0.237$은 값의 범위와 맞먹는 크기이며, 평가
문제에서 $\alpha=0.1$은 쓸 수 없는 설정이다.

\paragraph{왜 멈추는가.}
$1/n$ 평균은 $n$이 커지면 새 표본의 발언권이 $1/n$로 줄어들어 추정치가 한 점으로 굳는다. 고정값
$\alpha$는 그렇지 않다. 표본이 몇 개 쌓였든 새로 오는 $G$가 언제나 $\alpha$만큼의 발언권을 갖는다.
식~\eqref{eq:ema}을 $Q \leftarrow (1-\alpha)Q + \alpha G$로 고쳐 쓰고 $G$들이 서로 독립이며 분산이
$\sigma_G^2$라 두면, 정상 상태에서의 분산은
\begin{equation}
  \mathrm{Var}[Q_\infty] = \frac{\alpha}{2-\alpha}\,\sigma_G^2
  \label{eq:ema-var}
\end{equation}
이다. 이 값은 $n$과 무관하다. 아무리 오래 돌려도 추정치는 참값 둘레의 폭
$\sqrt{\alpha/(2-\alpha)}\,\sigma_G$짜리 띠 안에서 계속 서성인다. 뒤집어 말하면 지수 이동 평균이
실질적으로 들고 있는 표본 수는 $n_{\text{eff}} = (2-\alpha)/\alpha$로 고정되어 있다. $\alpha=0.1$이면
$19$개다. $100{,}000$ 에피소드를 돌려도 $19$개 분량만 들고 있는 셈이다.

표~\ref{tab:alpha}의 측정/이론이 $\alpha$ 세 값에서 $1.52$, $1.54$, $1.58$로 거의 같다는 것이 이
설명을 뒷받침한다. 상수배만큼 어긋나는 것은 앞 절 [보충]에서 본 그대로다.
식~\eqref{eq:ema-var}은 $G$들이 독립이라 가정했는데 한 에피소드 안의 방문들은 꼬리를 공유해 상관되어
있으므로 실제 분산이 그만큼 부풀려진다. 어긋남이 상수배에 그친다는 것은 $\alpha$에 대한 의존
...
그림~\ref{fig:alpha-rho} 왼쪽이 이 두 방식의 궤적을 직접 그린 것이다.
...
에피소드 길이와 중요도 가중치가 살아 있는 꼬리 길이의 분포. \S\ref{sec:offpolicy} 끝의 [보충]에서
다룬다. \texttt{ch05/paper/fig\_mc.py}에서 생성.}
\label{fig:alpha-rho}
\end{figure}
...
한 칸 차이가 나고 $10$칸을 모두 맞히는 비율은 세 배로 벌어진다. 두 표가 상반되어 보이지만
같은 성질의 양면이다. $1/n$ 평균이 잔차를 $0$으로 밀어 넣을 수 있는 것은 표본들이 모두 \emph{같은}
...
$\alpha$를 고르는 일이 곧 그 저울이다. 크면 빨리 잊는 대신 띠가 넓어지고, 작으면 띠는 좁아지되
...
실재한다는 것은 \S\ref{sec:variance}에서 평가 대상만 탐욕 정책으로 바꾸어 확인한다.

여기서 $\varepsilon$를 줄이면 될 것 같지만 그 길은 막혀 있다. $\varepsilon$를 줄이면 표의 차이는
작아지는 대신 \S\ref{sec:greedy-trap}에서 본 함정이 되살아난다. 탐색이 부족해져 방문하지 않는 칸이
...
$\varepsilon$-탐욕 정책이다. \S\ref{sec:eps-greedy}에서 \lstinline|greedy_probs()|에
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\linewidth]{{fig 5-19}.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.66\linewidth]{{fig 5-21}.png}
...
값이 $\pi$에서 확률을 갖고 있으면 그 몫은 표본에 영영 나타나지 않고, 식~\eqref{eq:is}의 나눗셈도
성립하지 않는다. 오프-정책 예제가 $b$로 $\varepsilon$-탐욕 정책을 고른 것도 그 조건을 맞추려는
것이다. 모든 행동에 최소
...
식~\eqref{eq:is}의 등호는 평균에 대한 것이지 흩어진 정도에 대한 것이 아니다.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\linewidth]{{fig 5-20}.png}
...
표본을 다시 뽑아 보면 세 방법 모두 추정치의 평균이 $2.7$에 붙어 있어 식~\eqref{eq:is}의 등호가
...
처방은 식~\eqref{eq:is}에서 바로 읽힌다. $\rho = \pi(x)/b(x)$가 $1$에서 멀어질수록 분산이 커지므로
...
식~\eqref{eq:is}는 확률 변수 하나에 대한 것이었다. 에피소드는 걸음이 여럿이고 각 걸음마다 행동을
고르므로, 수익 $G_t$의 가중치는 그 뒤 모든 걸음의 가중치를 곱한 것이 된다.
\begin{equation}
  \rho_{t} = \prod_{k=t}^{T-1} \frac{\pi(A_k \mid S_k)}{b(A_k \mid S_k)}.
  \label{eq:rho-prod}
\end{equation}
구현은 에피소드를 뒤에서부터 훑는 \S\ref{sec:backward}의 순서를 그대로 쓰면서 $\rho$를 곱해
쌓는 것으로 끝난다.
...
부호가 갈라지는 대목이 \S\ref{sec:on-off}에서 미뤄 둔 확인이다. 온-정책의 평균 차이는
...
식~\eqref{eq:rho-prod}의 곱 때문에 확률 변수 하나짜리 예제보다 사정이 나쁘다. 걸음이 늘수록
...
취하는 값은 셋뿐이다. 대상 정책이 \emph{결정적}이기 때문이다.

\lstinline|greedy_probs(Q, state, epsilon=0)|이 돌려주는 $\pi$는 한 행동에 $1$, 나머지 셋에 $0$을
준다. 행동 정책은 $\varepsilon=0.1$이므로 탐욕 행동에 $\varepsilon/4 + (1-\varepsilon) = 0.925$,
나머지에 $\varepsilon/4 = 0.025$를 준다. 한 걸음의 가중치 $\pi(a\mid s)/b(a\mid s)$가 가질 수 있는
값이 그 조합으로 정해진다.
\begin{equation}
...
세 번째 줄이 생기는 자리가 \S\ref{sec:eps-greedy}에서 사소해 보인다고 적어 둔 동점 처리다.
\lstinline|update()|는 \lstinline|greedy_probs()|를 $\pi$용과 $b$용으로 두 번 부르는데, 함수 안의
\lstinline|argmax()|는 동점이면 무작위로 하나를 고른다. 두 번의 호출이 서로 다른 행동을 뽑으면
$\pi$가 $1$을 준 행동에 $b$는 $0.025$만 주고, 가중치가 단숨에 $40$배로 뛴다. $Q$가 모두 $0$인
학습 초기에 네 행동이 전부 동점이므로 그 시기에 몰려서 나타난다.
...
$0$이 압도적이라는 사실이 이 알고리즘의 성격을 정한다. 식~\eqref{eq:rho-prod}의 곱은 한 번
$0$이 되면 그보다 앞선 걸음에서 영영 $0$이므로, $\rho$가 죽은 뒤의 갱신은
$G = \gamma \cdot 0 \cdot G + R_t = R_t$가 된다. 수익이 아니라 \emph{그 걸음의 즉시 보상 하나}로
$Q$를 갱신하는 셈이다. 시드 $10$개에서 그런 갱신이 전체의 $53.1\%$부터 $65.5\%$까지, 평균
$59.9\%$를 차지했다.

가중치가 살아 있는 구간이 얼마나 짧은지는 그림~\ref{fig:alpha-rho} 오른쪽에 있다. 에피소드 길이는
평균 $9.9$$\sim$$13.5$걸음에 중앙값 $5$걸음이고 시드에 따라 $20{,}000$걸음이 넘는 것도 나오는데,
$\rho \neq 0$인 꼬리는 평균 $4.65$걸음이고 시드 $10$개를 통틀어 $8$걸음을 넘은 적이 없다.
에피소드가 아무리 길어져도 가중치가 살아남는 구간의 길이는 거의 변하지 않는다.

두 방향의 결과가 표~\ref{tab:on-vs-off}에 함께 나타난 것이다. 남는 표본은 대상 정책이 실제로 걸을
길 위의 것뿐이라 정책의 순위는 잘 맞고, 그 표본의 수익이 $4.65$걸음에서 잘려 나가는 데다 드물게
$40$배가 섞여 들어오니 값의 추정은 흔들린다. 누적 $\rho$의 최댓값도 시드에 따라 $1.6$에서 $63.9$까지
튀었다.
...
판본을 쓴다. 두 파일의 동점 처리가 서로 다르다는 점은 \S\ref{sec:eps-greedy}에 적었고,
