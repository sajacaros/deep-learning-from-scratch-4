...
갱신식 안에는 환경 모델 $p$와 $r$가 그대로 들어 있었다. 현실의 문제는 그 표를 내주지 않는다.
\emph{몬테카를로법}(MC법)은 전제를 바꾸는 대신 계산을 바꾼다. 가치 함수는 수익의 기댓값이므로,
분포를 알아야만 구할 수 있는 것이 아니라 \emph{여러 번 해보고 평균을 내면} 된다.
...
아예 짚지 않은 다섯 자리에 논의를 더하였다. 절 구성은 원서 5장의 목차를 그대로 따르며, 더한 논의는
...
\caption{$\varepsilon$-탐욕 정책이 각 행동에 주는 확률. 막대 네 개가 모두 $\varepsilon/4$
이상이고, 최선의 행동만 거기서 $1-\varepsilon$만큼 더 높다. 확률이 $0$인 행동이 없으므로
...
\subsubsection{[보충] 고정값 \texorpdfstring{$\alpha$}{alpha}는 오차를 끝까지 줄이지 못한다}
\label{sec:alpha-residual}
...
\begin{itemize}[leftmargin=*,itemsep=3pt]
  ...
  \item \textbf{탐욕 정책만으로는 학습이 멈춘다.} 결정적 정책은 언제나 같은 길을 걸으므로 나머지
        칸의 $Q$가 갱신되지 않는다. $\varepsilon$-탐욕 정책은 모든 행동의 확률을
        적어도 $\varepsilon/4$로 남겨 둔다.
...
본문의 [보충] ``고정값 $\alpha$는 오차를 끝까지 줄이지 못한다''가 쓴 근거다. 제어를 잠시 밀어 두고 무작위 정책 평가를
