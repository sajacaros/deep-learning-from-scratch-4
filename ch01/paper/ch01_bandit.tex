...
% ---- 정책 흐름도 -----------------------------------------------------------
% 탐욕 정책과 eps-탐욕 정책은 「고르는 칸」 하나만 다르므로, 고리 전체를 매크로로
% 두고 그 칸에 붙는 설명만 인자로 받는다(두 그림의 배치가 어긋나지 않게 하려는 것).
\tikzset{
  loopbox/.style   = {draw, rounded corners=2pt, align=center,
                      minimum width=36mm, minimum height=8mm, font=\small},
  loopghost/.style = {loopbox, draw=black!35, dashed, text=black!55},
  looparr/.style   = {-{Latex[length=2mm]}, thick},
  looplbl/.style   = {font=\scriptsize, inner sep=3pt},
}
\newcommand{\policyloop}[1]{%
\begin{tikzpicture}[node distance=12mm]
  \node[loopbox]               (q)   {추정 $\Qe(a)$,\ \ $a = 1,\dots,10$};
  \node[loopbox,   below=of q] (sel) {기계 선택 $A_t$};
  \node[loopbox, below=of sel] (r)   {보상 $R_t \in \{0,1\}$};
  \node[loopghost, below=of r] (win) {관측 승률 $=$ 누적 보상 $/$ 걸음 수};
  \draw[looparr] (q)   -- node[looplbl, right] {#1} (sel);
  \draw[looparr] (sel) -- node[looplbl, right]
        {참값 $\qt(A_t)$의 확률로 당첨 \ (\,$\qt$는 환경만 안다\,)} (r);
  \draw[looparr] (r)   -- node[looplbl, right] {기록만 한다} (win);
  \draw[looparr] (r.west) -- ++(-14mm,0) |- (q.west);
  \node[looplbl, left, align=right]
        at ($(q.west)!0.5!(r.west) + (-14mm,0)$)
        {update\\[2pt] 고른 기계의 $\Qe(A_t)$만 갱신};
\end{tikzpicture}}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{성능 상한의 측정}(\S\ref{sec:ceiling}). 참값 $\qt$를 그대로 아는 에이전트를 같은 잣대로
        측정하여, 승률이라는 잣대 자체가 어디서 한계에 닿는지를 수치로 제시한다.
\end{enumerate}
...
탐욕 정책만 쓸 때 한 걸음이 어떻게 도는지를 그림~\ref{fig:greedy-loop}에 정리하였다.

\begin{figure}[htbp]
\centering
\policyloop{$\arg\max_a \Qe(a)$ — 지금 가장 커 보이는 기계}
\caption{탐욕 정책의 한 걸음. 점선 상자는 기록용 지표이며, 에이전트는 이 값을 참조하지 않는다.}
\label{fig:greedy-loop}
\end{figure}

그림에서 짚어둘 것이 셋이다. 첫째, 추정치 $\Qe$는 슬롯머신마다 하나씩 있다. 슬롯머신이 10대면
$\Qe$도 10개이며, 서로 견주어 골라야 하므로 후보마다 점수가 하나씩 필요하다. 둘째, 참값 $\qt$는
고리 안에 직접 등장하지 않는다. 에이전트는 $\qt$를 볼 수 없고, $\qt$가 정한 확률로 나온 보상 $R$만
받는다. 셋째, 관측 승률은 고리를 여러 번 돈 결과를 기록한 값일 뿐 $\Qe$가 아니다. 이 값은 실험자가
성능을 재려고 남기는 것이지 에이전트가 참조하는 값이 아니다.
...
그림~\ref{fig:greedy-loop}에서 바뀌는 것은 고르는 칸 하나뿐이다(그림~\ref{fig:eps-greedy-loop}).

\begin{figure}[htbp]
\centering
\policyloop{\shortstack[l]{%
  확률 $1-\eps$ : $\arg\max_a \Qe(a)$ \ (활용)\\[2pt]
  확률 $\eps$ \ \ \ : 무작위 — $\Qe$를 보지 않는다 \ (탐색)}}
\caption{$\eps$-탐욕 정책의 한 걸음. 그림~\ref{fig:greedy-loop}과 견주면 행동을 고르는 칸만 다르다.}
\label{fig:eps-greedy-loop}
\end{figure}

$\Qe$를 보고 고르는 것은 확률 $1-\eps$일 때뿐이고, 확률 $\eps$일 때는 $\Qe$를 아예 무시한다. 나머지,
곧 플레이에서 보상을 거쳐 갱신에 이르는 부분은 탐욕 정책과 같다. 무작위로 고른 걸음도 보상을 받고
갱신을 거치므로, 탐색은 결국 그 기계의 $\Qe$를 고치는 데 쓰인다.
...
즉 상위 $\alpha$들은 \emph{이미 상한에 닿아 있다}. 표~\ref{tab:alpha-grid}의 오른쪽 세 열이 붙어 있는
것은 $\alpha$들이 대등해서가 아니라 \emph{승률이라는 잣대가 이미 한계에 닿았기 때문}이다. 행동을 $\arg\max_a \Qe(a)$로 고르는 이상
