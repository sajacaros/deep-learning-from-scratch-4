...
- 탐욕 정책만 쓰면 한 걸음이 이렇게 돎
```
  추정 Q    슬롯머신마다 하나씩, 총 10개
    │
    │  argmax — 지금 가장 커 보이는 기계를 고른다
    ▼
  기계 선택
    │
    │  그 기계의 참값 q 확률로 당첨   (q는 환경만 안다)
    ▼
  보상 R    0 또는 1
    │
    ├──→  update — 선택한 기계의 Q를 관측된 보상 R을 반영해 업데이트한다 → 맨 위로 돌아감
    │
    ▼
  관측 승률   누적 보상 ÷ 걸음 수
```
- 그림에서 짚을 점 셋
  - **`Q`는 슬롯머신마다 하나씩** 있음(10대면 10개) — 서로 비교해서 골라야 하므로 후보마다 점수가 필요
  - **참값 `q`는 고리 안에 직접 등장하지 않음** — 에이전트는 `q`를 볼 수 없고, `q`가 정한 확률로 나온 보상 `R`만 받음
  - **관측 승률은 고리를 여러 번 돈 결과물이지 `Q`가 아님** — 기록만 할 뿐 에이전트는 이 값을 보지도 않음
...
```
  추정 Q    슬롯머신마다 하나씩, 총 10개
    │
    │  확률 1-ε : argmax — 가장 커 보이는 기계   (활용)
    │  확률  ε  : 무작위 — Q를 아예 보지 않는다   (탐색)
    ▼
  기계 선택
    │
    │  그 기계의 참값 q 확률로 당첨   (q는 환경만 안다)
  ...
  관측 승률   누적 보상 ÷ 걸음 수
```
  - `Q`를 보고 고르는 건 확률 `1-ε`일 때뿐이고, 확률 `ε`일 때는 `Q`를 무시함
  - 나머지(플레이 → 보상 → 갱신)는 탐욕 정책과 똑같음
  - 무작위로 고른 걸음도 보상을 받고 `update`를 하므로, 탐색은 **그 기계의 `Q`를 고치는 데 쓰임**
## 1.4 밴디트 알고리즘 구현
### 1.4.1 슬롯머신 구현
- `Bandit` 클래스 — 환경 역할
...
- ![](images/s04_epsilon_comparison.png)
  - **왼쪽 그림**(1,000걸음): 위아래로 갈라진 **두 무리**를 봄
    - 맨 아래 깔린 두 선이 `ε=0`과 `ε=1`(둘 다 0.49 근처) → 한쪽만 극단적으로 쓰면 출발선에서 못 벗어남
...
- 왼쪽 아래 그림을 숫자로 옮긴 콘솔 출력
  ...
  - 그 성능 상한을 실제로 재본 콘솔 출력
    ...
    - 그래서 그 위로는 승률에 차이가 날 자리가 없음 — 오른쪽 세 열이 붙어 있는 것은 `α`들이 비슷해서가 아니라 **승률이라는 잣대가 이미 한계에 닿았기 때문**
