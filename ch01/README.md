...
- ![](../images/figures/fig%201-13.png)
  - **축**: x = 걸음 수, y = 그때까지의 **평균 승률**(누적보상 ÷ 걸음 수)
  - **볼 곳**: 왼쪽 끝의 심한 출렁임과, 오른쪽으로 갈수록 잦아드는 모습
  - 초반에는 표본이 적어 승률이 크게 튀지만, 200걸음 이후 0.8 위로 올라가 안정됨
...
- 왼쪽 아래 그림을 숫자로 옮긴 콘솔 출력
  ...
  - 오른쪽 세 열(`α`=0.1/0.3/0.8)이 거의 붙어 있음 → **승률로는 `α`의 우열을 못 가림**
    - `ε` 때문에 성능 상한이 0.9505로 낮고, 행동은 `argmax`라 `Q`의 **순위**만 맞으면 보상이 같기 때문
    - `α`를 고르려면 `|Q - q|`(1.4.1) 같은 추정 오차를 재야 함 — 참값 `q`를 아는 시뮬레이션에서만 가능
  - 행끼리 비교 금지 — `σ`가 크면 승률이 1을 넘는 슬롯머신이 생겨 점수의 상한 자체가 올라감
## 1.6 정리
- 강화 학습은 **에이전트와 환경의 상호작용**에서 얻는 **보상**을 단서로 학습
- **밴디트 문제**는 상태가 변하지 않는 가장 단순한 강화 학습 문제
